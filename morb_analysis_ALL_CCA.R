#morb analysis PVD PSL Anameia - prevalnce data CCA ≥3
...
data$CCA_pos <-as.factor(data$CCA_pos)
#data$MalariaYN
#data$CCA_pos
#data$age
#data$HW_pos
...
#Concurvity diagnostics were performed to assess the degree of non-linear dependency among predictors in the GAM model. Concurvity is analogous to multicollinearity in linear models but applies to smooth terms, indicating how much one predictor (or smooth) can be approximated by others. Values close to 1 suggest high dependency, while values near 0 indicate independence. In this model, concurvity estimates were low for both smooth terms: s(CCA_pos) had an estimated concurvity of 0.08 (observed = 0.04), and s(age) had an estimated concurvity of 0.09 (observed = 0.06). The worst-case concurvity values (maximum at any point in the data) were also acceptably low (0.31–0.34). The parametric terms showed slightly higher concurvity (0.41), but this remained well below commonly used thresholds for concern (e.g., >0.7). These results suggest that the predictors in the model are sufficiently independent, and there is no evidence of problematic concurvity affecting model stability or interpretability.
...
plot(simulateResiduals(mod4)) #good


#best fit model
PSL_mod_ps <- gam(PSL_scoreBIN ~ 
                    s(age, bs="ps") + HW_pos + MalariaYN+CCA_pos+sex,
                  data = PSLmodData, family = "binomial", method = "REML")
...
mod4<-gam(AnaemiaYN ~ 
            s(age, bs="cr") + HW_pos + MalariaYN+CCA_pos+sex,
          data = ANmodData, family = "binomial", method = "REML")
#
#
#
